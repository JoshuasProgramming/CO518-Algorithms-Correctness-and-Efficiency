...
import comp5180a12022.DynamicArray;
...
public class MyUoKDynamicArray extends DynamicArray {
    ...
    @Override
    boolean isFullQ1() {
        // fill in details. You can add methods to this class as you see necessary, as
        // long as this method functions correctly

        DynamicArray array = new DynamicArray();
        int count = 0;
        
        int hiddenLength = hiddenArray.length;
        ...
        System.out.println("nah count: " + count + " array cap: " + capacity);
        return false;
    }
    ...
    void addItemQ2(int item, double x) {
        ...
        // int copy_add_item_counter = 0;
        // int capSize = capacity * (int) Math.round(x);
        // System.out.print(capSize + " " + x);
        if (isFullQ1()) {
            
            int[] tempArray = hiddenArray;
            hiddenArray = new int[capacity * (int) Math.ceil(x)];
            for(int i = 0; i < capacity; i++){
                hiddenArray[i] = tempArray[i];
                opCountForAdds++;
            }
            capacity = capacity * (int) Math.ceil(x);
        }
    ...
    public static void main(String[] args) {
        MyUoKDynamicArray mda = new MyUoKDynamicArray(5);

        // stick some dummy data into
        System.out.println(mda.toString());
        mda.addItem(22);
        System.out.println(mda.toString());
        mda.addItemQ2(33, 1.5);
        System.out.println(mda.toString());
